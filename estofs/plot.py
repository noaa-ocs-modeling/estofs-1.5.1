...
def setStationPlot (master, stationName, cwl, n, oldxlim, ylim):
    ...
    if not np.isnan(datum_mhhw_m) and datum_mhhw_m < ylim[1]:
        ...
        ax.text(cwl['time'][-80], datum_mhhw_m + 0.05, 'MHHW',color='c',fontsize=7)
    ...
    if not np.isnan(datum_mllw_m) and datum_mllw_m > ylim[0] and datum_mllw_m < ylim[1]:
        ...
        ax.text(cwl['time'][-80], datum_mllw_m + 0.05, 'MLLW',color='c',fontsize=7)
    ...
    if not np.isnan(datum_hat_m) and datum_hat_m < ylim[1]:
        ...
        ax.text(cwl['time'][-60], datum_hat_m  + 0.05, 'HAT',color='y',fontsize=7)

    ax.plot(xlim[1], 0, 'dk',color='k')
    ax.text(cwl['time'][-120], 0.05, 'LMSL',color='k',fontsize=7)
...
def stations (cwlFile, htpFile, pp, titleStr, plotPath, args):
    ...
    for n in range(nStations):
        ...
        ax.text(xlim[0],ylim[1]+0.1,'NOAA/NOS')
